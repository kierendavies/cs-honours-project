...
\begin{proof}
  ...
  The inner-most block is evaluated less than $n^2$ times, which is finite because the number of arguments $n$ is finite, and each evaluation terminates because \textproc{testSubClassOf} was shown to terminate.  Therefore the entire algorithm terminates.
\end{proof}
...
\end{theorem}
\begin{proof}
  Correctness is clearly seen from the definition of $\mathtt{DisjointUnion}$.  Termination is trivial.
\end{proof}
