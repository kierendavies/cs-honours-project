...
\todo[Check if necessary; possibly - an abstract was specifically called for but not an introduction. It may be that the following section suffices as an intro though needs to be spruced up]

\section{Project Description}

\subsection{Project Background}

\todo[Which part of the broader problem do you intend to address?]
\todo[Reference better]
...
\textsc{Scone} \todo[a little more description would have been better. Like, scone does x; we aim to do y, which precedes/serves at the back end of x] is an ontology testing tool which is oriented towards behaviour-driven development \cite{SconeUserGuide}.  We do not consider it because it uses a sophisticated domain-specific language to specify tests, and focusses on testing human-understandable scenarios rather than complex axioms.
...
Currently, only a small number of unit tests have been defined at all \todo["not a small number: we have 42 of them, for each of the SROIQ features."]. Their correctness has only been proven empirically. Proof of correctness thus needs to be done. There needs to exist a larger set of unit tests to cover a greater variety of types of axioms to improve the test-driven development methodology scope.

Benchmarking would help to inform choices of reasoner not just within the scope of test-driven development but also ontology development. It also helps to improve the speed performance of the unit tests by matching reasoners to axioms more appropriately. \todo["benefits are not a problem statement; ... especially taking onto account the language of the ontology"]

\section{Aims and Solutions}
\todo[See if this needs to be done at all; may not be necessary as it might be covered under the headings we already have]

\section{Project Design}

\subsection{Procedures and Methods}

To devise new unit tests, we will draw inspiration from TDDOnto.  We will include some of those \todo["only some of them?"] which TDDOnto explicitly excludes, such as RBox tests, and also extrapolate to more complex compound axioms.  We will also introduce more unit test outcomes by characterising failure modes; for example, when vocabulary is missing, or when the missing axiom may be added to the ontology without causing inconsistency.
...
\todo[so, the performance tester waits for the proofs? I suggest parallelizing things.]
...
\todo[these are materials, not ethical etc issues.]
